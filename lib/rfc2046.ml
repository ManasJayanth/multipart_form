...
(* From RFC 2046 bchars := bcharsnospace / " " *)
let is_bchars = function ' ' -> true | c -> is_bcharsnospace c

(* From RFC 2046

   dash-boundary := "--" boundary ; boundary taken from the value of ; boundary
   parameter of the ; Content-Type field. *)
...
let discard_all_to_dash_boundary boundary =
  ...
  | Some '-' ->
    ...
    | false ->
      advance 1 *> m )
  | Some _ ->
    advance 1 *> m (* impossible case? *)
  | None ->
    return ()

(* From RFC 2046

   transport-padding := *LWSP-char ; Composers MUST NOT generate ; non-zero
   length transport ; padding, but receivers MUST ; be able to handle padding ;
...
let body_part body =
  Rfc2045.headers >>= fun fields ->
  (Rfc822.crlf *> return `CRLF <|> return `Nothing <* commit >>= function
   | `CRLF ->
     body fields >>| Option.some
   | `Nothing ->
     return None)
...
let epilogue parent =
  match parent with
  | Some boundary ->
    discard_all_to_delimiter boundary
  | None ->
    skip_while (fun _ -> true)
