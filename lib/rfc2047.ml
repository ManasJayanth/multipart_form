...
let charset_of_uppercase_string x =
  ...
  | "csASCII" ->
    `US_ASCII
  | x ->
    (try (Rosetta.encoding_of_string x :> charset) with
    | Invalid_argument _ ->
      (match Uutf.encoding_of_string x with
      | Some (#uutf_charset as charset) ->
        charset
      | _ ->
        `Charset x))

let charset_of_string raw =
  match raw with
  | "US-ASCII"
  | "iso-ir-6"
  | "ANSI_X3.4-1968"
  ...
  | "csASCII" ->
    raw, `US_ASCII
  | x ->
    (try raw, (Rosetta.encoding_of_string x :> charset) with
    | Invalid_argument _ ->
      (match Uutf.encoding_of_string x with
      | Some (#uutf_charset as charset) ->
        raw, charset
      | _ ->
        raw, charset_of_uppercase_string x))
...
(* From RFC 2047 token = 1*<Any CHAR except SPACE, CTLs, and especials> *)
let token =
...
let normalize_quoted_printable_with_rosetta ?(chunk = 512) ~charset raw =
  ...
  | `Malformed data ->
    Rresult.R.error_msgf "Malformed input: %S" data
  ...
  | `US_ASCII ->
    ...
    and go_qp_decode () =
      ...
      | `Await ->
        assert false
      | `Char chr ->
        go_encode (`Uchar (Uchar.of_char chr))
      | `End ->
        go_encode `End
      | `Malformed _ as v ->
        v
...
let normalize_base64 ?chunk ~charset raw =
  ...
  | `US_ASCII ->
    ...
    Base64.decode raw
  | #uutf_charset as charset ->
    normalize_base64_with_uutf ?chunk ~charset raw
  | #Rosetta.encoding as charset ->
...
let normalize ?chunk ~charset ~encoding raw =
  ...
  | Base64 ->
    normalize_base64 ?chunk ~charset raw
...
(* From RFC 2047
   ...
   encoded-word = "=?" charset "?" encoding "?" encoded-text "?=" charset =
   token ; see section 3 encoding = token ; see section 4

   Both 'encoding' and 'charset' names are case-independent. Thus the charset
   name "ISO-8859-1" is equivalent to "iso-8859-1", and the encoding named "Q"
   may be spelled either "Q" or "q".

   About charset

   The 'charset' portion of an 'encoded-word' specifies the character set
   associated with the unencoded text. A 'charset' can be any of the character
   set names allowed in an MIME "charset" parameter of a "text/plain" body part,
   or any character set name registered with IANA for use with the MIME
   text/plain content-type.

   About encoding

   Initially, the legal values for "encoding" are "Q" and "B". These encodings
   are described below. The "Q" encoding is recommended for use when most of the
   characters to be encoded are in the ASCII character set; otherwise, the "B"
   encoding should be used. Nevertheless, a mail reader which claims to
   recognize 'encoded-word's MUST be able to accept either encoding for any
   character set which it supports.

   See [pecu], [rosetta] and [ocaml-base64] for more details about encoding. *)
let encoded_word =
  string "=?" *> token >>| charset_of_string >>= fun (charset_raw, charset) ->
  ...
  string "?="
  *> return { charset; encoding; raw = charset_raw, encoding_raw, raw; data }
